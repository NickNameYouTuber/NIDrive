...
class StorageService:
    ...
    def check_connection(self) -> bool:
        ...
        Raises:
            Exception: Если не удалось подключиться к MongoDB
        """
        try:
            # Проверяем подключение, запрашивая информацию о сервере
...
class InMemoryStorageService:
    """
    Резервная реализация хранилища в памяти для случаев, когда MongoDB недоступна.
    Данные не сохраняются между перезапусками сервера.
    """
    
    def __init__(self, max_user_storage: int = 5 * 1024 * 1024 * 1024):
        """
        Инициализация хранилища в памяти
        
        Args:
            max_user_storage: Максимальный размер хранилища пользователя (по умолчанию 5GB)
        """
        self.max_user_storage = max_user_storage
        self.files = {}  # Словарь для хранения файлов и метаданных: {file_id: (metadata, content)}
    
    def check_connection(self) -> bool:
        """Проверка соединения всегда успешна для хранилища в памяти"""
        return True
    ...
    def get_user_storage_info(self, user_id: str) -> StorageInfo:
        ...
        Returns:
            StorageInfo: Информация о хранилище
        """
        used = sum(meta.size for meta, _ in self.files.values() 
    ...
    async def save_file(self, 
                 ...
                 is_public: bool = False) -> FileMetadata:
        """
        Сохранить файл в хранилище в памяти
        ...
        try:
            ...
            # Сохраняем файл и метаданные в памяти
            self.files[file_id] = (metadata, content)
            
            return metadata
            
        except Exception as e:
            logger.error(f"Ошибка при сохранении файла в памяти: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Не удалось сохранить файл: {str(e)}")
        finally:
            await file.seek(0)  # Сбрасываем позицию в файле на начало для возможного повторного чтения
    ...
    def get_file(self, file_id: str, user_id: Optional[str] = None, access_token: Optional[str] = None) -> tuple:
        """
        Получить файл из хранилища в памяти
        ...
        metadata, content = self.files[file_id]
        ...
        if not metadata.is_public:
            ...
            if user_id and metadata.user_id != user_id:
                if not access_token or metadata.access_token != access_token:
                    raise HTTPException(status_code=403, detail="Доступ запрещен")
        
        # Возвращаем файл и метаданные
        return io.BytesIO(content), metadata
    
    def list_user_files(self, user_id: str, folder: Optional[str] = None) -> List[FileMetadata]:
        """
        Получить список файлов пользователя из хранилища в памяти
        ...
        for metadata, _ in self.files.values():
            if metadata.user_id == user_id:
                if folder is None or metadata.folder == folder:
    ...
    def get_folders(self, user_id: str) -> List[str]:
        """
        Получить список папок пользователя из хранилища в памяти
        ...
        for metadata, _ in self.files.values():
            if metadata.user_id == user_id and metadata.folder:
    ...
    def delete_file(self, file_id: str, user_id: str) -> bool:
        """
        Удалить файл из хранилища в памяти
    ...
    def set_file_privacy(self, file_id: str, user_id: str, is_public: bool) -> FileMetadata:
        """
        Изменить статус приватности файла в хранилище в памяти
